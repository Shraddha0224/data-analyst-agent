...
async def analyze(questions_txt: UploadFile = File(...), dataset: UploadFile = File(...)):
    # Read uploaded files
    questions_content = await questions_txt.read()
    data_content = await dataset.read()

    # For example: parse CSV into DataFrame
    df = pd.read_csv(io.BytesIO(data_content))

    # Just a placeholder
    return {"questions": questions_content.decode("utf-8"), "rows": len(df)}
